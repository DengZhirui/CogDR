...
class Dataloader(object):
    def __init__(self, args, data, device):
        ...
        self.item_category_dic, self.category_dic, self.category_num = self.read_category(self.category_path)
        ...
        self.user2category_path = '../datasets/' + args.dataset + '/user2category.txt'
        self.user2category = np.loadtxt(self.user2category_path, dtype=np.float, delimiter=',', unpack=False)
        self.kg_emb_path = ['../datasets/'+args.dataset+'/kgdata/entity2idx.pkl', '../datasets/'+args.dataset+'/kgdata/embeddings/transr/ent_embedding.tsv']
        self.user_kg_emb, self.item_kg_emb = self.load_kg_embedding(self.kg_emb_path)

    def load_kg_embedding(self, path):
        with open(path[0], 'rb') as f:
            entitiy2idx = pickle.load(f)

        ent_embedding = np.loadtxt(path[1], delimiter='\t')

        user_kg_emb = []
        for i in range(self.user_number):
            user_kg_emb.append(ent_embedding[entitiy2idx['U'+str(i)]])

        item_kg_emb = []
        for i in range(self.item_number):
            item_kg_emb.append(ent_embedding[entitiy2idx['I'+str(i)]])

        array_user, array_item = np.array(user_kg_emb), np.array(item_kg_emb)
        norm_user = array_user / np.linalg.norm(array_user, axis=1)[:, np.newaxis]
        norm_item = array_item / np.linalg.norm(array_item, axis=1)[:, np.newaxis]

        return torch.tensor(norm_user).float(), torch.tensor(norm_item).float()
        #return torch.tensor(np.array(user_kg_emb)), torch.tensor(np.array(item_kg_emb))
            ...
            for line in tqdm(lines):
                ...
                for c in category:
                    if c not in category_d:
                        ...
                        category_d[c] += 1
                dic[item] = torch.tensor(category)
        return dic, category_d, num

    def get_sample_weight(self, category_dic):
        #dic = {}
        #categories = category_dic.values()
        category_list = [(key, category_dic[key]) for key in category_dic]
        category_list.sort(key=lambda x:x[0])
        category_list = [val[1] for val in category_list]
        weight_tensor = torch.tensor(category_list, dtype = torch.float)
        ...
        # find category with the minimize number for each item as their weight
        item_cate_min = {}
        for item in self.item_category_dic:
            cate_min = -1
            cate_count_min = 999999999
            for cate in self.item_category_dic[item].tolist():
                if category_dic[cate] < cate_count_min:
                    cate_count_min = category_dic[cate]
                    cate_min = cate
            item_cate_min[item] = cate_min
        
        # sort item
        item_list = [(key, item_cate_min[key]) for key in item_cate_min]
        item_list.sort(key=lambda x:x[0])
        item_list = [val[1] for val in item_list]
        return weight[torch.tensor(item_list)]#weight[torch.tensor(list(category_dic.values()))]
    ...
    def read_train_graph(self, path):
        ...
        category_tensor = torch.nn.utils.rnn.pad_sequence(list(self.item_category_dic.values()), batch_first=True,  padding_value=-2)
