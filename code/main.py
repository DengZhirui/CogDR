...
if __name__ == '__main__':
    ...
    model = choose_model(args, dataloader).float()
    model = model.to(device)
    
    opt = torch.optim.Adam(model.parameters(), lr = args.lr, weight_decay = args.weight_decay)
    early_stop(99999.99, model)
    args.category_balance = False

    for epoch in range(args.epoch):
        model.train()

        loss_train = torch.zeros(1).to(device)
        loss_train_contrast = torch.zeros(1).to(device)
        ...
        for i in range(args.neg_number):
            ...
            score_pos, score_neg, loss_contrast = model(graph_pos, graph_neg)
            loss_train_contrast += loss_contrast
        ...
        loss_train = loss_train / args.neg_number + loss_train_contrast / args.neg_number * args.loss_weight
        ...
        score_pos, score_neg, loss_contrast = model(graph_val_pos, graph_val_neg)
        if not args.category_balance:
        ...
        loss_val = loss_val + loss_contrast * args.loss_weight
